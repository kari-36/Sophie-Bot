...
async def report1_cmd(message: Message, chat: Chat, strings: Dict[str, str]):
    # Checking whether report is disabled in chat!
    check = await db.disabled.find_one({'chat_id': chat.id})
    if check:
...
@get_strings_dec('reports')
async def report2_cmd(message: Message, chat: Chat, strings: Dict[str, str]):
    await report(message, chat, strings)


async def report(message: Message, chat: Chat, strings: Dict[str, str]):
    user = message.from_user.id

    if (await is_user_admin(chat.id, user)) is True:
    ...
    if (await is_user_admin(chat.id, offender_id)) is True:
